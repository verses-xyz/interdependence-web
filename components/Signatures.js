      ...
      content={sigs.map((sig, index) => <div className="font-mono" key={sig.SIG_HANDLE}>
        <div className={"space-x-0 flex py-4 md:space-x-4 sm:py-2 md:py-4 overflow-hidden" + (index === sigs.length - 1 ? "" : " border-b")}>
          <h3 className="py-2 text-left">
            ...
            <div className="inline-block -my-2 py-1 px-3 ml-3 text-sm rounded-3xl text-gray-120 bg-gray-200 overflow-hidden md:visible">
          ...
          <div className="py-2">
            <div className="inline-block -my-2 py-1 px-4 text-sm rounded-3xl text-gray-120 bg-gray-200 overflow-hidden md:visible">
              {sig.SIG_SIGNATURE.slice(0, 10)}&hellip;
            </div>
          </div>

          {/*
           // previous code for arweave signatures
          <h3 className="py-2 text-left">
            {sig.SIG_HANDLE ?
          ...
          <a
            target="_blank"
            className="invisible py-2 px-4 rounded-3xl text-gray-120 bg-gray-200 overflow-hidden md:visible"
            href={`https://etherscan.io/address/${sig.SIG_ADDR}`}>{
              // cleanHandle(sig.SIG_HANDLE, sig.SIG_ADDR, sig.SIG_ISVERIFIED) :
              sig.SIG_ADDR.slice(0, 6)
            } </a>
                         */}
        </div>
      </div>)}

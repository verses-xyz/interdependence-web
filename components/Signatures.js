    ...
    <div className="px-10">
      {sigs.map(sig => <div className="font-mono" key={sig.SIG_HANDLE}>
        ...
        <div className="space-x-0 flex border-b py-4 md:space-x-4 sm:py-1 md:py-4 overflow-hidden">
          <h3 className="py-2">
            <a className="hover:underline" href={`https://twitter.com/${sig.SIG_HANDLE}`}>{sig.SIG_NAME}</a>
          </h3>
          <div className="flex-1"/>
          <a
            className="invisible py-2 px-4 rounded-3xl text-brown-120 bg-gray-200 hover:text-brown-20 overflow-hidden md:visible"
            href={`https://viewblock.io/arweave/address/${sig.SIG_TX}`}> @{cleanHandle(sig.SIG_HANDLE)} </a>
          <a
            className="invisible py-2 px-4 rounded-3xl text-brown-120 bg-gray-200 hover:text-brown-20 overflow-hidden lg:visible"

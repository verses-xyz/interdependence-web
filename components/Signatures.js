...
export default function Signatures({ txId, sigs, setSigs }) {
  ...
  return (
    <Box
      ...
      content={
        <>
          {sortedSigs.map((sig, index) => (
            <div className="font-mono w-full" key={sig.SIG_ADDR}>
              <div
                className={
                  "space-x-0 flex py-4 sm:space-x-4 sm:py-2 w-auto sm:py-4 overflow-hidden" +
                  (index === sigs.length - 1 && reachedEnd
                    ? ""
                    : " border-b border-gray-wash")
                }
              >
                <h3 className="py-2 text-left">
                  {sig.SIG_HANDLE ? (
                    <a
                      ...
                      href={`https://twitter.com/${sig.SIG_HANDLE}`}
                    >
                      {sig.SIG_NAME || "Anonymous"}
                    </a>
                  ) : (
                    sig.SIG_NAME
                  )}
                ...
                {sig.SIG_ID && (
                  <a
                    ...
                    href={`https://arweave.net/tx/${sig.SIG_ID}`}
                  >
                    tx:{sig.SIG_ID.slice(0, 6)}
                  </a>
                )}
                ...
                <a
                  ...
                  href={
                    sig.SIG_HANDLE
                      ? `https://twitter.com/${sig.SIG_HANDLE}`
                      : null
                  }
                  className="sm:hover:bg-gray-hover transition duration-250 ease-in-out flex row items-center text-sm px-1 sm:px-4  text-white bg-white bg-opacity-10 text-opacity-60 overflow-hidden"
                >
                  {sig.SIG_ISVERIFIED && (
                    <div className="mr-2">
                      <Checkmark filled />
          ...
          {!reachedEnd && (
            <div
              className="font-mono w-full cursor-pointer"
              key="load_more"
              onClick={fetchMore}
            >
